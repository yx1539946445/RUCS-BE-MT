...
from models.hcmt_unet import Unet
...
parser.add_argument('--model', type=str, default='HCMT', help='model_name')
...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            ema_outputs_0 = torch.softmax(ema_outputs[0], dim=1)
            ema_outputs_1 = torch.softmax(ema_outputs[1], dim=1)
            ema_outputs_2 = torch.softmax(ema_outputs[2], dim=1)
            ema_outputs_3 = torch.softmax(ema_outputs[3], dim=1)
            w0 = 0.5
            w1 = 0.4
            w2 = 0.05
            w3 = 0.05
            # loss sup
            loss_sup_0 = 0.5*(loss_func(outputs[0][:labeled_bs],label_batch[:labeled_bs])+dice_loss(outputs_soft_0[:labeled_bs],label_batch[:labeled_bs].unsqueeze(1)))
            loss_sup_1 = 0.5*(loss_func(outputs[1][:labeled_bs],label_batch[:labeled_bs])+dice_loss(outputs_soft_1[:labeled_bs],label_batch[:labeled_bs].unsqueeze(1)))
            loss_sup_2 = 0.5*(loss_func(outputs[2][:labeled_bs],label_batch[:labeled_bs])+dice_loss(outputs_soft_2[:labeled_bs],label_batch[:labeled_bs].unsqueeze(1)))
            loss_sup_3 = 0.5*(loss_func(outputs[3][:labeled_bs],label_batch[:labeled_bs])+dice_loss(outputs_soft_3[:labeled_bs],label_batch[:labeled_bs].unsqueeze(1)))
            loss_sup = w0 * loss_sup_0 + w1 * loss_sup_1 + w2 * loss_sup_2 + w3*loss_sup_3
            # loss unsup
            loss_unsup_0 = torch.mean((outputs_soft_0[labeled_bs:]-ema_outputs_0) ** 2)
            loss_unsup_1 = torch.mean((outputs_soft_1[labeled_bs:]-ema_outputs_1) ** 2)
            loss_unsup_2 = torch.mean((outputs_soft_2[labeled_bs:]-ema_outputs_2) ** 2)
            loss_unsup_3 = torch.mean((outputs_soft_3[labeled_bs:]-ema_outputs_3) ** 2)
            loss_unsup = w0 * loss_sup_0 + w1 * loss_sup_1 + w2 * loss_sup_2 + w3 * loss_sup_3
