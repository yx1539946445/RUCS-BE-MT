...
class Dataset(BaseDataSet):
    ...
    def _set_files(self):
        ...
        root = "../"
        if self.split == "val":
            file_list = os.path.join(root, "data/csv_file", f"{self.split}" + ".txt")
        elif self.split in ["train_supervised", "train_unsupervised"]:
            file_list = os.path.join(root, "data/csv_file",
...
class data_image(BaseDataLoader):
    def __init__(self, kwargs, ddp_training=False, dgx=False):
        ...
        self.dataset = Dataset(ddp_training, **kwargs, dgx=dgx)
        if ddp_training:
            train_sampler = torch.utils.data.distributed.DistributedSampler(self.dataset)
        else:
            train_sampler = None
        super(data_image, self).__init__(self.dataset, self.batch_size, shuffle, num_workers, val_split=None,
                                         sampler=train_sampler)


import json

config = json.load(open("../configs/config_net.json"))

supervised_loader = data_image(config['train_supervised'], ddp_training=True, dgx=False)
unsupervised_loader = data_image(config['train_unsupervised'], ddp_training=True, dgx=False)

val_loader = data_image(config['val_loader'], dgx=True)
