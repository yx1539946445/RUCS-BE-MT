...
from models.dct_unet import Unet
...
parser.add_argument('--model', type=str, default='DCT', help='model_name')
...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        ...
        for i_batch, sampled_batch in enumerate(train_loader):
            ...
            outputs_tanh, outputs = model(volume_batch_weak)
            outputs_soft = torch.sigmoid(outputs)

            with torch.no_grad():
                gt_dis = compute_sdf(label_batch[:].cpu(
                ).numpy(), outputs[:labeled_bs,1,...].shape)
                gt_dis = torch.from_numpy(gt_dis).float().to(device)
            loss_sdf = mse_loss(outputs_tanh[:labeled_bs ,1, ...], gt_dis)
            loss_seg = ce_loss(outputs[:labeled_bs,1,...], label_batch[:labeled_bs].float())
            loss_seg_dice = losses.dice_loss(
                outputs_soft[:labeled_bs,1,...], label_batch[:labeled_bs] == 1)
            dis_to_mask = torch.sigmoid(-1500*outputs_tanh)

            loss_unsup = torch.mean((dis_to_mask - outputs_soft) ** 2)
            loss_sup = loss_seg_dice + 0.3 * loss_sdf
        ...
        if iter_num > 0 and iter_num % 200 == 0:
            ...
            with torch.no_grad():
                ...
                for sampled_batch in val_loader:
                    ...
                    outputs_tanh,outputs = model(image)
